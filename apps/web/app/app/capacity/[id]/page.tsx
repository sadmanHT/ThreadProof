...
export default async function CapacityDetailPage({ params }: Props) {
  ...
  if (!active || active.organization.role !== "factory") notFound();
  ...
  const [{ data: factory }, { data: credential }, { data: proofs }, { data: allocations }] = await Promise.all([
    ...
    supabase.from("proof_jobs").select("id,status,circuit_version,order_version_id,created_at,started_at,completed_at,chain_tx_hash,chain_block_number,error_code").eq("capacity_opening_id", opening.id).eq("factory_organization_id", active.organization_id).order("created_at", { ascending: false }).limit(12),
    supabase.from("capacity_allocations").select("id,chain_allocation_id,order_version_id,old_commitment,new_commitment,order_commitment,chain_tx_hash,chain_block_number,confirmed_at,created_at").eq("capacity_opening_id", opening.id).order("created_at", { ascending: false }).limit(12),
  ]);
  ...
  return (
    <div className="workspace-page">
      ...
      <header className="proof-detail-hero"><div><span className="kicker">CAPACITY STATE EVIDENCE</span><h1>{factory?.display_name ?? "Factory"} · {opening.period_id}</h1><p>{titleCase(opening.process_id)} · circuit v{opening.circuit_version} · active context {active.organization.display_name}</p></div><StatusBadge value={opening.status} /></header>

      <section className="privacy-banner"><span className="privacy-icon">◌</span><div><strong>The opening is inspectable; the amount is not.</strong><p>This page intentionally excludes encrypted remaining-capacity and randomness fields. It is also bound to the selected factory membership, so switching organizations is required before another factory opening can be inspected.</p></div></section>
      ...
      <section className="proof-evidence-grid">
        <article className="panel"><div className="panel-heading"><div><span className="kicker">PROOF ACTIVITY</span><h2>Jobs bound to this opening</h2></div><span className="panel-count">{proofs?.length ?? 0}</span></div>{(proofs ?? []).length ? <div className="record-list">{(proofs ?? []).map((proof) => <Link className="record-row" href={`/app/proofs/${proof.id}`} key={proof.id}><div><strong>Proof {shortHash(proof.id, 8, 6)}</strong><span>circuit v{proof.circuit_version} · created {formatDate(proof.created_at)}{proof.chain_block_number ? ` · block ${proof.chain_block_number}` : ""}</span></div><StatusBadge value={proof.status} /></Link>)}</div> : <div className="empty-state"><strong>No proof jobs in this factory context</strong><span>Proof jobs appear here when this opening is used by the selected factory in a feasible-capacity workflow.</span></div>}</article>

        <article className="panel"><div className="panel-heading"><div><span className="kicker">CONFIRMED TRANSITIONS</span><h2>Canonical allocations</h2></div><span className="panel-count">{allocations?.length ?? 0}</span></div>{(allocations ?? []).length ? <div className="record-list">{(allocations ?? []).map((allocation) => <div className="record-row" key={allocation.id}><div><strong>{allocation.chain_allocation_id ? `Allocation ${shortHash(allocation.chain_allocation_id, 10, 8)}` : `Transition ${shortHash(allocation.id, 8, 6)}`}</strong><span>old {shortHash(allocation.old_commitment)} → new {shortHash(allocation.new_commitment)} · block {allocation.chain_block_number ?? "not indexed"}</span><span className="mono">{allocation.chain_allocation_id ?? "Awaiting CapacityAllocationRecorded event binding"}</span></div><StatusBadge value={allocation.chain_allocation_id && allocation.confirmed_at ? "confirmed" : "reconciling"} /></div>)}</div> : <div className="empty-state"><strong>No confirmed allocations indexed</strong><span>An active opening may have no spend yet. A successful proof job alone is not evidence that CapacityVault accepted a transition or recorded a reusable allocation reference.</span></div>}</article>
      </section>

      <section className="panel trust-panel"><span className="kicker">AUTHORITY BOUNDARY</span><h2>CapacityVault decides which commitment and allocation are canonical.</h2><p>Active-organization scope prevents accidental cross-membership composition, but it is not a source of protocol authority. Supabase remains encrypted witness storage and a rebuildable mirror; allocation identifiers are copied only from matching CapacityAllocationRecorded events.</p></section>
    </div>
  );
}
