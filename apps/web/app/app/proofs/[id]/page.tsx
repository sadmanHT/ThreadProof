...
export default async function ProofDetailPage({ params }: Props) {
  ...
  const [{ data: version }, { data: opening }, { data: verifierRows }] = await Promise.all([
    ...
    supabase.from("private_capacity_openings").select("id,period_id,process_id,chain_state_key,policy_hash,circuit_version,status,last_chain_block,updated_at").eq("id", job.capacity_opening_id).maybeSingle(),
  ...
  const { data: order } = version
    ? await supabase.from("purchase_orders").select("id,external_reference,title,status,current_version,chain_order_id").eq("id", version.purchase_order_id).maybeSingle()
    : { data: null };
  ...
  return <div className="workspace-page">
    ...
    <header className="proof-detail-hero"><div><span className="kicker">PROOF-OF-FEASIBLE-CAPACITY</span><h1>{order?.title || order?.external_reference || "Proof evidence"}</h1><p>{order?.external_reference ? `${order.external_reference} · ` : ""}{version ? `order version ${version.version} · ` : ""}circuit v{job.circuit_version}</p></div><StatusBadge value={job.status} /></header>
    ...
    <section className="proof-evidence-grid">
      <article className="panel"><div className="panel-heading"><div><span className="kicker">BOUND STATEMENTS</span><h2>Order and capacity context</h2></div></div><dl className="definition-grid"><div><dt>Order</dt><dd>{order?.external_reference ?? shortHash(job.order_version_id)}</dd></div><div><dt>Order status</dt><dd>{order ? <StatusBadge value={order.status} /> : "Not visible"}</dd></div><div><dt>Order version</dt><dd>{version ? `v${version.version}` : "Not visible"}</dd></div><div><dt>Order commitment</dt><dd className="mono">{version ? shortHash(version.order_commitment) : "—"}</dd></div><div><dt>Capacity period</dt><dd>{opening?.period_id ?? "Not visible"}</dd></div><div><dt>Process</dt><dd>{opening ? titleCase(opening.process_id) : "Not visible"}</dd></div><div className="wide"><dt>Capacity state key</dt><dd className="mono hash-full">{opening?.chain_state_key ?? "Not visible"}</dd></div><div><dt>Opening mirror state</dt><dd>{opening ? <StatusBadge value={opening.status} /> : "Not visible"}</dd></div><div><dt>Last opening block</dt><dd>{opening?.last_chain_block ?? "—"}</dd></div></dl></article>

      <article className="panel trust-panel"><span className="kicker">PRIVACY BOUNDARY</span><h2>The witness is not on this page.</h2><p>ThreadProof stores encrypted remaining-capacity and randomness separately. This evidence view reads the proof job, public statements, visible order metadata and the current indexed opening reference. It never queries <span className="mono">proof_job_private_state</span>.</p><div className="privacy-access-list"><span className="privacy-chip private">Witness concealed</span><span className="privacy-chip shared">Public inputs verifiable</span><span className="privacy-chip consortium">Chain result attributable</span></div></article>
    </section>
    ...
    <p className="footnote">A proof object is not a capacity authorization by itself. Canonical feasibility depends on successful verification against the provenance-bound verifier and the current CapacityVault commitment state.</p>
  </div>;
}
