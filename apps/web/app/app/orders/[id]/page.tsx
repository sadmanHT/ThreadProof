...
import { ConfirmSubmitButton, PendingSubmitButton } from "@/components/pending-submit-button";
...
export default async function OrderDetailPage({ params, searchParams }: Props) {
  ...
  const canAuthorize = canOperate && !blockingCancellation && !!order.factory_organization_id && ["draft", "proposed", "feasible", "infeasible"].includes(order.status);
  const canCancel = canOperate && !activeAuthorization && !blockingCancellation && order.current_version > 0 && ["proposed", "feasible", "infeasible", "accepted"].includes(order.status);
  ...
  return (
    <div className="workspace-page">
      ...
      {blockingCancellation && blockingCancellation.status !== "confirmed" ? <div className="alert alert-warning">Cancellation for anchored version {blockingCancellation.expected_version} is {blockingCancellation.status}. This order remains canonically active until OrderCancelled is indexed.</div> : null}
      ...
      <section className="detail-grid">
        <article className="panel">
          <div className="panel-heading"><div><span className="kicker">COMMERCIAL METADATA</span><h2>Private order summary</h2></div></div>
          <dl className="definition-grid"><div><dt>Quantity</dt><dd>{formatQuantity(order.quantity, order.unit)}</dd></div><div><dt>Category</dt><dd>{order.product_category || "—"}</dd></div><div><dt>Requested delivery</dt><dd>{formatDate(order.requested_delivery_date)}</dd></div><div><dt>Created</dt><dd>{formatDate(order.created_at)}</dd></div></dl>
        </article>
        <article className="panel trust-panel">
          <span className="kicker">CANONICAL AUTHORIZATION</span><h2>{order.current_version > 0 ? `Version ${order.current_version} anchored` : "Not yet anchored"}</h2>
          <p>{canonicalCancelled ? "OrderRegistry emitted OrderCancelled. Old version history remains immutable, but the current order can no longer authorize capacity spending or subcontracting." : order.current_version > 0 ? "The application mirrors the current OrderRegistry authorization. A capacity proof must bind to this exact current commitment." : "This row is an application draft only. It cannot authorize production or consume certified capacity until a buyer-signed OrderRegistry event is indexed."}</p>
          <dl className="definition-grid"><div><dt>Chain order ID</dt><dd className="mono">{shortHash(order.chain_order_id)}</dd></div><div><dt>Order commitment</dt><dd className="mono">{shortHash(order.current_order_commitment)}</dd></div><div><dt>Policy hash</dt><dd className="mono">{shortHash(order.current_policy_hash)}</dd></div></dl>
        </article>
      </section>

      {canEditDraft ? <section className="panel form-panel"><div className="panel-heading"><div><span className="kicker">BUYER DRAFT</span><h2>Edit operational details</h2></div></div><form className="stack-form" action={updateOrderAction}><input type="hidden" name="orderId" value={order.id} /><div className="field-grid two"><label>External reference<input name="externalReference" required defaultValue={order.external_reference} /></label><label>Title<input name="title" required defaultValue={order.title ?? ""} /></label></div><div className="field-grid three"><label>Product category<input name="productCategory" defaultValue={order.product_category ?? ""} /></label><label>Quantity<input name="quantity" type="number" min="0.001" step="0.001" required defaultValue={order.quantity ?? undefined} /></label><label>Unit<input name="unit" required defaultValue={order.unit ?? "pieces"} /></label></div><label>Requested delivery<input name="requestedDeliveryDate" type="date" defaultValue={order.requested_delivery_date ?? ""} /></label><div className="form-actions"><PendingSubmitButton className="button secondary" pendingLabel="Saving draft…">Save draft</PendingSubmitButton></div></form><form action={deleteOrderAction} className="danger-zone"><input type="hidden" name="orderId" value={order.id} /><div><strong>Discard draft</strong><span>Only version-zero drafts can be deleted. Click delete once, then confirm the destructive action.</span></div><ConfirmSubmitButton className="button danger" pendingLabel="Deleting draft…" confirmLabel="Confirm delete">Delete draft</ConfirmSubmitButton></form></section> : null}
