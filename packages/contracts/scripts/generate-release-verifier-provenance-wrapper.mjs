...
const verifierSourcePath = join(generatedDir, "CapacityReleaseVerifier.sol");
const wrapperSourcePath = join(generatedDir, "CapacityReleaseVerifierWithProvenance.sol");
const manifestPath = join(proofDir, "release_verifier_provenance.json");
...
if (contractNames.length !== 1 || !contractNames[0] || !verifierSource.includes("verifyProof")) {
  throw new Error("Expected exactly one generated CapacityRelease verifier contract exposing verifyProof");
}

const circuitArtifactPath = join(proofDir, "CapacityRelease.r1cs");
const verificationKeyPath = join(proofDir, "release_verification_key.json");
...
const wrapperContract = "CapacityReleaseVerifierWithProvenance";

const wrapperSource = `// SPDX-License-Identifier: GPL-3.0
pragma solidity ^0.8.28;

import {${generatedVerifierContract}} from "./CapacityReleaseVerifier.sol";

/// @notice CI-only wrapper binding the generated release verifier to the exact development R1CS and verification key.
...
const manifest = {
  ...
  circuitVersion: 1,
  circuitArtifact: {
    path: "CapacityRelease.r1cs",
    keccak256: circuitArtifactHash,
  },
  verificationKey: {
    path: "release_verification_key.json",
...
console.log(`THREADPROOF_RELEASE_VERIFIER_PROVENANCE ${JSON.stringify(manifest)}`);
