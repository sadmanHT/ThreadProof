-- Follow-up to the CapacitySpent recovery projection after the large-integer storage migration.
-- ThreadProof stores cryptographic scalars as canonical decimal text to avoid JavaScript/SQL
-- precision loss. Keep the recovery projection in the same representation end-to-end.
...
begin
  ...
  begin
    ...
    new_commitment := new.data ->> 'newCommitment';
    order_commitment := new.data ->> 'orderCommitment';
    spend_nullifier := new.data ->> 'nullifier';
  ...
  if old_commitment is null or old_commitment !~ '^[0-9]+$'
     or new_commitment is null or new_commitment !~ '^[0-9]+$'
     or order_commitment is null or order_commitment !~ '^[0-9]+$'
     or spend_nullifier is null or spend_nullifier !~ '^[0-9]+$'
     or event_order_id is null or event_order_id !~ '^0x[0-9a-fA-F]{64}$'
     or event_circuit_version <= 0
  then
    raise warning 'CapacitySpent event % has invalid scalar/order/circuit metadata; private mirror unchanged', new.transaction_hash;
    return new;
  end if;
  ...
  where pj.chain_tx_hash = new.transaction_hash
     or (
       ...
       and pj.public_inputs #>> '{signals,8}' = spend_nullifier
     )
  ...
  if coalesce(matched_job.public_inputs #>> '{signals,5}', '') <> old_commitment
     or coalesce(matched_job.public_inputs #>> '{signals,6}', '') <> new_commitment
     or coalesce(matched_job.public_inputs #>> '{signals,7}', '') <> order_commitment
     or coalesce(matched_job.public_inputs #>> '{signals,8}', '') <> spend_nullifier
...
'Crash-safe private mirror reconciliation for canonical CapacitySpent events using canonical decimal-text scalar storage. Never fabricates missing witness openings.';

-- Re-run already-indexed spends after replacing the function. The trigger is unchanged.
