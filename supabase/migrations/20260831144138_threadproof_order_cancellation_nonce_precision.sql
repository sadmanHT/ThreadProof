-- Preserve exact uint256 cancellation nonces across PostgREST/JavaScript boundaries.
alter table public.order_cancellation_jobs
  alter column nonce type text using nonce::text;
alter table public.order_cancellation_jobs
  add constraint order_cancellation_jobs_nonce_decimal_format
    check (nonce ~ '^[0-9]+$');
...
comment on column public.order_cancellation_jobs.nonce is
'Exact decimal uint256 buyer nonce stored as text to avoid PostgREST/JavaScript precision loss.';
